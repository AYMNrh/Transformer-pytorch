...
# CONNECTION OF THE ENCODER DIFFERENT LAYERS
class ResidualConnection(nn.Module):
	...
	def forward(self, x, sublayer):
		# in paper they apply fisrt normalizatin then sublayer
...
class ProjectionLayer(nn.Module):
	...
	def __init__(self, d_mdodel: int, vocab_size: int) -> None:
		super().__init__()
		self.proj = nn.Linear(d_mdodel, vocab_size)
...
# function to build the transformer and init all prams
def build_transformer(src_vocab_size: int, tgt_vocab_size: int, 
                      src_seq_len: int, tgt_seq_len: int, 
                      d_model: int = 512, N: int = 6, h: int = 8, dropout: float = 0.1, d_ff: int = 2048) -> Transformer: # params taken from attention is all you need
	...
	# Creating the projectionlayer
	projection_layer = ProjectionLayer(d_model, tgt_vocab_size)
 
	# Creating the transformer
	transformer = Transformer(encoder, decoder, src_embed, tgt_embed, src_pos, tgt_pos, projection_layer)
 
	# init the params using the xavier algo
